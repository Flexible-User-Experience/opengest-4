...
class OperatorDocumentationPdfManager
{
    ...
    public function buildSingle(Operator $operator, $documents): TCPDF
    {
        $this->pdfEngineService->initDefaultPageEngineWithTitle('Operario: '.$operator);
        $pdf = $this->pdfEngineService->getEngine();

        return $this->buildOnePayslipPerPage($operator, $documents, $pdf);
    }

    public function outputSingle(Operator $operator, $documents): string
    {
        $pdf = $this->buildSingle($operator, $documents);

        return $pdf->Output('Documentacion operario '.$operator->getId().'.pdf', 'I');
    }

    private function buildOnePayslipPerPage(Operator $operator, $documents, TCPDF $pdf): TCPDF
    {
        // add start page
